class array:
    def __init__ (self,size,items):
        ...
        if list(map(type,items)).count(list(map(type,items))[0]) == len(items) and self.size>len(items):
            ...
            raise Exception("TypeError: item data types do not match")
    def append(self,item):
        if type(item) == self.typey and len(self.array) < self.size:
            self.array.append(item)
        elif len(self.array) < self.size:
            raise Exception("BufferError: Overflow error")
        else:
            raise Exception("TypeError: item data type does not match")
